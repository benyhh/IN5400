...
class ImageCaptionModel(nn.Module):
    def __init__(self, config: dict):
        ...
        # Done TODO: The output layer (final layer) is a linear layer. What should be the size (dimensions) of its output?
        ...
        # Done TODO: Check the task description and replace None with the correct input layer

        self.input_layer = nn.Sequential(
            nn.Dropout(p = 0.25),
            nn.Conv1d(2048,512, kernel_size = 1, stride = 1, padding = 0, dilation = 1),
            nn.BatchNorm1d(512),
            nn.LeakyReLU()
            )
    ...
    def forward(self, cnn_features, x_tokens, is_train: bool, current_hidden_state=None) -> tuple:
        ...
        if current_hidden_state is None:
            # Done TODO: Initialize initial_hidden_state with correct dimensions depending on the cell type.
            ...
            if self.cell_type == "LSTM":
                initial_hidden_state = torch.zeros(self.num_rnn_layers, cnn_features.shape[0], 2 * self.hidden_state_sizes).to("cuda:0")
            else:
                initial_hidden_state = torch.zeros(self.num_rnn_layers, cnn_features.shape[0], self.hidden_state_sizes).to("cuda:0")
        else:
        ...
        logits, hidden_state = self.rnn(x_tokens, processed_cnn_features, initial_hidden_state, self.output_layer,
                                        self.embedding_layer, self.attention_layer, is_train)
...
class RNNOneLayerSimplified(nn.Module):
    ...
    def forward(self, tokens, processed_cnn_features, initial_hidden_state, output_layer: nn.Linear,
        ...
        # Done TODO: Fetch the first (index 0) embeddings that should go as input to the RNN.
        ...
        for i in range(sequence_length):
            update_state = torch.zeros_like(current_hidden_state)
            ...
            # Done TODO: Create the input for the RNN cell
            ...
            # Done TODO: Call the RNN cell with input_for_the_first_layer and current_hidden_state as inputs
            input_for_the_first_layer = torch.cat((input_tokens,processed_cnn_features), dim=1)
            update_state[0,:] = self.cells[0](input_for_the_first_layer, current_hidden_state[0])

            current_hidden_state = update_state
...
class RNN(nn.Module):
    def __init__(self, input_size, hidden_state_size, num_rnn_layers, last_layer_state_size = 512, cell_type='GRU'):
        """
        ...
        #Max pool function
        self.maxpool1d = nn.MaxPool1d(kernel_size=7, stride = 6, padding = 1, dilation = 1)

        # Done TODO: len(input_size_list) == num_rnn_layers and input_size_list[i] should contain the input size for layer i.
        ...
        input_size_list.append(last_layer_state_size+hidden_state_size)
        ...
        if self.cell_type == "RNN":
            self.cells = nn.ModuleList([RNNsimpleCell(hidden_state_size, input_size) for input_size in input_size_list])
        elif self.cell_type == "LSTM":
            self.cells = nn.ModuleList([LSTMCell(hidden_state_size, input_size) for input_size in input_size_list])
        elif self.cell_type == "GRU":
            self.cells = nn.ModuleList([GRUCell(hidden_state_size, input_size) for input_size in input_size_list])
        else:
            raise ValueError(f"Choose valid cell_type, cell_type = {cell_type}.")
    ...
    def forward(self, tokens, processed_cnn_features, initial_hidden_state, output_layer: nn.Linear,
        ...
        maxpooled_features = self.maxpool1d(processed_cnn_features).squeeze(2)
        ...
        for i in range(sequence_length):
            ...
            input_for_the_first_layer = torch.cat((input_tokens,maxpooled_features), dim=1)

            update_state[0] = self.cells[0](input_for_the_first_layer, current_hidden_state[0])

            for layer in range(1, self.num_rnn_layers-1):

                update_state[layer] = self.cells[layer](update_state[layer-1,:,:self.hidden_state_size], current_hidden_state[layer])                 

            #Put hidden and cell state through attention layer
            attention_weights = attention_layer(update_state[-2])
            attention_sum = torch.matmul(processed_cnn_features, attention_weights.unsqueeze(2)).squeeze(2)

            #Concatenate last layer hidden state and attention weights
            hidden_attention = torch.cat((update_state[-2,:,:self.hidden_state_size], attention_sum), dim = 1)

            update_state[-1] = self.cells[-1](hidden_attention, current_hidden_state[-1])

            current_hidden_state = update_state

            logits_i = output_layer(update_state[-1,:,:self.hidden_state_size])
            ...
            if i < sequence_length - 1:
                if is_train:
                    ...
                    # Done TODO: Compute predictions above and use them here by replacing None with the code in comment
                    input_tokens = embedding_layer(predictions)
...
class GRUCell(nn.Module):
    def __init__(self, hidden_state_size: int, input_size: int):
        ...
        # Done TODO: Initialise weights and biases for the update gate (weight_u, bias_u), reset gate (w_r, b_r) and hidden
        #       state (weight, bias).
    ...
    def forward(self, x, hidden_state):
        ...
        """
        # Done TODO: Implement the GRU equations to get the new hidden state and return it
        x2 = torch.cat((x, hidden_state), dim = 1)
        #Update gate
        zt = torch.sigmoid( torch.mm(x2, self.weight_u) + self.bias_u )

        #Reset gate
        rt = torch.sigmoid( torch.mm(x2, self.weight_r) + self.bias_r )

        #Proposed activation
        x3 = torch.cat((x, rt*hidden_state), dim = 1)
        ht_hat = torch.tanh( torch.mm(x3, self.weight) + self.bias )

        #Final output activation
        new_hidden_state = zt * hidden_state + (1-zt) * ht_hat
...
class LSTMCell(nn.Module):
    def __init__(self, hidden_state_size: int, input_size: int):
        ...
        # Done TODO: Initialise weights and biases for the forget gate (weight_f, bias_f), input gate (w_i, b_i),
        ...
        self.bias_i = nn.Parameter(2 * torch.ones(1, hidden_state_size))
        ...
        self.bias_o = nn.Parameter(2 * torch.ones(1, hidden_state_size))
        # Memory cell parameters
        self.weight = nn.Parameter(torch.randn(input_size + hidden_state_size, hidden_state_size) / np.sqrt(input_size + hidden_state_size))
        self.bias = nn.Parameter(2 * torch.ones(1, hidden_state_size))
    ...
    def forward(self, x, hidden_state):
        ...
        """
        # Done TODO: Implement the GRU equations to get the new hidden state, cell memory and return them.
        #       The first half of the returned value must represent the new hidden state and the second half
        #       new cell state.

        #Extract hidden state and cell state from input
        ht = hidden_state[:,:-self.hidden_state_size]
        Ct = hidden_state[:,-self.hidden_state_size:]

        #Combine input and hidden state
        x2 = torch.cat((x,ht), dim = 1)

        #Forget gate equation
        ft = torch.sigmoid( torch.mm(x2, self.weight_f) + self.bias_f)

        #Input gate equation
        it = torch.sigmoid( torch.mm(x2, self.weight_i) + self.bias_i)

        #Memory cell equation
        C_hat = torch.tanh( torch.mm(x2, self.weight) + self.bias)

        #Output gate equation
        ot = torch.sigmoid( torch.mm(x2, self.weight_o) + self.bias_o)
        ...
        new_hidden_state = torch.cat((ht,Ct), dim = 1)
