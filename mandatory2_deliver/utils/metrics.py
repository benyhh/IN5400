# from https://github.com/SunJiamei/LRP-imagecaptioning-pytorch/blob/master/models/metrics.py
...
class Score:
    ...
    def calculate(self, id_to_prediction, id_to_references, tokenize=True):
        # id_to_preds = {}
        # for id_, pred in id_to_prediction.items():
        #     id_to_preds[id_] = [pred]
        #id_to_prediction_old = id_to_prediction 
        #id_to_references_old = id_to_references       
        if True == tokenize:
            ...
            for id_, pred in id_to_prediction.items():
                id_to_preds[id_] = [   ] 
                for p in pred:
                    id_to_preds[id_].append(p['caption'])
            id_to_prediction = id_to_preds    

            id_to_refs = {}
            for id_, pred in id_to_references.items():
                id_to_refs[id_] =[]
                for p in pred:
                    id_to_refs[id_].append(p['caption'])
            id_to_references = id_to_refs    
        '''           
                          
        #print(id_to_prediction_old)   
        #print(id_to_prediction) 
        print(id_to_references_old)   
        print(id_to_references)               
        exit()    
        '''
